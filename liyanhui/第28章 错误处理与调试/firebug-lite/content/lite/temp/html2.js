...
FBL.ns(function() {
  with(FBL) {
      ...
      getContextMenuItems: function(node, target) {
        ...
        if (node && node.nodeType == 1) {
          ...
          if (getAncestorByClass(target, "nodeAttr")) {
            ...
            items.push({
              label: $STRF("EditAttribute", [attrName]),
              nol10n: true,
              command: bindFixed(this.editAttribute, this, node, attrName)
            }, {
              label: $STRF("DeleteAttribute", [attrName]),
              nol10n: true,
          ...
          if (!(nonEditableTags.hasOwnProperty(node.localName))) {
            items.push(
              "-", { label: "EditElement", command: bindFixed(this.editNode, this, node) }, { label: "DeleteElement", command: bindFixed(this.deleteNode, this, node) }
            );
          }

        } else {
          items.push(
            "-", { label: "EditNode", command: bindFixed(this.editNode, this, node) }, { label: "DeleteNode", command: bindFixed(this.deleteNode, this, node) }
          );
        }
    ...
    Firebug.HTMLPanel.CompleteElement = domplate(FirebugReps.Element, {
      tag: DIV({ "class": "nodeBox open $object|getHidden repIgnore", _repObject: "$object", role: 'presentation' },
    ...
    Firebug.HTMLPanel.SoloElement = domplate(Firebug.HTMLPanel.CompleteElement, {
      tag: DIV({ "class": "soloElement", onmousedown: "$onMouseDown" },
    ...
    Firebug.HTMLPanel.Element = domplate(FirebugReps.Element, {
      tag: DIV({ "class": "nodeBox containerNodeBox $object|getHidden repIgnore", _repObject: "$object", role: "presentation" },
        DIV({ "class": "nodeLabel", role: "presentation" },
    ...
    Firebug.HTMLPanel.TextElement = domplate(FirebugReps.Element, {
      tag: DIV({ "class": "nodeBox textNodeBox $object|getHidden repIgnore", _repObject: "$object", role: 'presentation' },
        DIV({ "class": "nodeLabel", role: "presentation" },
    ...
    Firebug.HTMLPanel.EmptyElement = domplate(FirebugReps.Element, {
      tag: DIV({ "class": "nodeBox emptyNodeBox $object|getHidden repIgnore", _repObject: "$object", role: 'presentation' },
        DIV({ "class": "nodeLabel", role: "presentation" },
          SPAN({ "class": "nodeLabelBox repTarget", role: 'treeitem' },
            "&lt;",
            SPAN({ "class": "nodeTag" }, "$object.localName|toLowerCase"),
    ...
    Firebug.HTMLPanel.AttrNode = domplate(FirebugReps.Element, {
      ...
      Firebug.HTMLPanel.TextNode = domplate(FirebugReps.Element, {
        tag: DIV({ "class": "nodeBox", _repObject: "$object" },
        ...
        charIterator: function(node) {
          ...
          for (var i = 0; i < str.length; i++) {
            ...
            switch (char) {
              ...
              case '\u200B':
                arr[i] = '\\u200B';
                break; // Zero width space http://www.fileformat.info/info/unicode/char/200b/index.htm
              default:
                arr[i] = '?';
                break;
            }
    ...
    HTMLEditor.prototype = domplate(Firebug.BaseEditor, {
      ...
      saveEdit: function(target, value, previousValue) {
        ...
        var first = this.editingElements[0],
          last = this.editingElements[1];
        if (last && last != first) {
    ...
    function findNodeAttrBox(objectNodeBox, attrName) {
      ...
      for (; child; child = child.nextSibling) {
        if (hasClass(child, "nodeAttr") && child.childNodes[1].firstChild && child.childNodes[1].firstChild.nodeValue == attrName) {
          return child;
        }
      }
    }
    ...
    function NodeSearch(text, doc, panelNode, ioBox) {
      ...
      this.findNextMatch = function(reverse, caseSensitive) {
        ...
        function walkNode() {
          return reverse ? walker.previousNode() : walker.nextNode(); }
      ...
      this.findNextInnerMatch = function(reverse, caseSensitive) {
        if (this.lastRange) {
          ...
          if (lastMatchNode.nodeType == Node.ATTRIBUTE_NODE && this.lastMatch.isValue == reverse) {
